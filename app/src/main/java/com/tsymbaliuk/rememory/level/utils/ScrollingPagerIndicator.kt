package com.tsymbaliuk.rememory.level.utils
...
class ScrollingPagerIndicator(
    ...
    constructor(context: Context, @Nullable attrs: AttributeSet?) : this(
        context,
        attrs,
        R.attr.scrollingPagerIndicatorStyle
    )
    ...
    fun onPageScrolled(page: Int, offset: Float) {
        ...
        if (!looped || 2 <= itemCount && itemCount <= visibleDotCount) {
            dotScale!!.clear()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val scaleDistance =
            (spaceBetweenDotCenters + (dotSelectedSize - dotNormalSize) / 2) * 0.7f
    ...
    private fun initDots(itemCount: Int) {
        ...
        firstDotOffset =
            (if (looped && this.itemCount > visibleDotCount) 0 else dotSelectedSize / 2.toFloat()) as Float
        visibleFrameWidth =
    ...
    private fun adjustFramePosition(offset: Float, pos: Int) {
        if (itemCount <= visibleDotCount) {
            ...
            val lastCenteredDot =
                getDotOffsetAt(dotCount - 1 - firstCenteredDotIndex)
            ...
            // Looped with scroll
            val center =
                getDotOffsetAt(infiniteDotCount / 2) + spaceBetweenDotCenters * offset
            visibleFramePosition = center - visibleFrameWidth / 2
        }
    }
